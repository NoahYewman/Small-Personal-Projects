...
def play_again():
    ...
    if again.lower()[0] == "y":
        guess_word = choose_word(site_scrape())
        word_definition = word_def(site_scrape())
        board = create_board(guess_word)
        game(board, guess_word, word_definition)

    else:
        print("Thanks for playing!")


def game(board, word, word_definition):
    user_attempts = 0
    previous_guesses = []
    guessed = False
...
word_data = site_scrape()
guess_word = choose_word(word_data)
word_definition = word_def(word_data)
board = create_board(guess_word)
game(board, guess_word, word_definition)
